...
const DistributionModule: React.FC<DistributionModuleProps> = ({ totalZakat }) => {
  ...
  const calculateDistribution = () => {
    ...
    const isFakirMiskinChecked = checkedMustahiks.includes('fakir') || checkedMustahiks.includes('miskin');
    const isAmilOnly = checkedMustahiks.length === 1 && checkedMustahiks[0] === 'amil';
    const isSkippingPriors = !isFakirMiskinChecked && checkedMustahiks.length > 0 &&
      (checkedMustahiks.includes('fi_sabilillah') || checkedMustahiks.includes('ibnu_sabil') || checkedMustahiks.includes('muallaf'));

    let hasError = false;
    if (isAmilOnly) {
      setValidationMessage('Distribusi tidak sesuai syariah: Zakat tidak boleh dibagikan ke Amil saja. Prioritas utama adalah Fakir & Miskin.');
      hasError = true;
    }
    if (isSkippingPriors) {
      setValidationMessage('Distribusi tidak sesuai syariah: Golongan Fakir dan Miskin adalah prioritas utama dan tidak boleh dilewati jika masih ada.');
      hasError = true;
    }
  ...
  return (
    <div className="bg-white shadow-lg rounded-xl p-6 md:p-8 mb-6">
      <h2 className="text-2xl font-bold mb-4 text-gray-700">3. Distribusi Zakat ke Mustahik (Level 10)</h2>

      <div className="bg-gray-100 border border-gray-300 p-4 rounded-lg mb-6">
        <h3 className="font-bold text-lg text-gray-800">Dalil Distribusi Zakat (QS. At-Taubah [9]: 60)</h3>
        <blockquote className="italic text-gray-700 mt-2">
          "Sesungguhnya zakat itu hanyalah untuk (1) orang-orang fakir, (2) orang-orang miskin, (3) amil zakat, (4) orang-orang yang dilunakkan hatinya (muallaf), (5) untuk (memerdekakan) hamba sahaya, (6) untuk (membebaskan) orang-orang yang berutang, (7) untuk jalan Allah (fi sabilillah), dan (8) untuk orang-orang yang sedang dalam perjalanan..."
        </blockquote>
      </div>

      <p className="mb-4 text-gray-600">Silakan centang golongan mustahik yang akan Anda beri zakat. Sistem akan menghitung alokasi dana dan memberikan validasi syariah.</p>

      {validationMessage && (
        <div className="bg-red-50 border border-red-300 text-red-700 p-4 rounded-lg mb-6 font-semibold">
          <strong>⚠️ {validationMessage}</strong>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
        ...
        <MustahikCard
          ...
          onCheckChange={(checked) => handleCheckChange('fi_sabilillah', checked)}
        />
        <MustahikCard
          id="ibnu_sabil"
